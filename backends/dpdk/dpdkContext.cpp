...
//    std::map <cstring, struct actionAttributes> actionAttrMap;
    ...
    add_space(out, 12); out << "\"name\": \"" <<  mainTableAttr.controlName + "." + tbl->name.originalName << "\",\n";
    ...
    if (structure->member_tables.count(tbl->name)) {
        ...
        add_space(out, 20); out << "\"name\": \"" <<  tableAttr.controlName + "." + memberTable->name.originalName <<"\",\n";
    ...
    if (structure->group_tables.count(tbl->name)) {
        ...
        out << "\n";
        add_space (out, 16); out << "{\n";
        add_space(out, 20); out << "\"name\": \"" << tableAttr.controlName + "." + groupTable->name.originalName <<"\",\n";
        add_space(out, 20); out << "\"handle\": " <<  tableAttr.tableHandle <<"\n";
        add_space (out, 16); out << "}\n";
        add_space(out, 12);
    }
